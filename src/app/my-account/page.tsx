...
// Define these outside the main component to avoid redefining them on every render
   ...
   <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      {animeList.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-x-4 gap-y-6">
              {animeList.map((item: UserAnimeListItem) => { // Added type
      ...
      <h2 className="text-2xl font-bold">Account Settings</h2>
       ...
       <div className="bg-card-dark p-6 rounded-lg shadow-md">
           ...
           <div className="space-y-3 max-h-60 overflow-y-auto pr-2">
               {receipts.length > 0 ? receipts.map((r: Receipt) => ( // Added type
                  <div key={r.id} className="p-3 bg-gray-700 rounded-md flex justify-between items-center gap-4">
                      ...
                      {(r.status === 'pending' || r.status === 'rejected') && (
                          <button
                              ...
                              disabled={loading} // Rely on main loading state passed as prop
...
export default function MyAccountPage() {
    ...
    const setupUser = useCallback(async (user: User) => {
        ...
        try {
            ...
            // Update State ONLY after all fetches are successful
    ...
    const checkSessionAndSetup = useCallback(async (isRetry = false) => {
        ...
        try {
            ...
            if (currentSession && currentSession.user) {
                ...
                await setupUser(currentSession.user); // Await data fetching
            } else {
            ...
            // ALWAYS set loading to false after the attempt completes
            setLoading(false);
        }
    ...
    useEffect(() => {
        ...
        checkSessionAndSetup(); // Call on initial mount
        ...
        const { data: authListener } = supabase.auth.onAuthStateChange(
            (event, newSession) => {
                ...
                if (newUserId !== previousUserId) {
                    console.log(`User change detected (${event}). Triggering checkSessionAndSetup.`);
                    checkSessionAndSetup(); // Let checkSessionAndSetup handle loading/error/data fetching
                } else {
                    console.log(`Auth event '${event}' occurred, user ID (${newUserId}) unchanged. No reload triggered.`);
                    // Optional: Handle USER_UPDATED silently
                }
            }
        );
    ...
    }, [checkSessionAndSetup, session]); // Dependencies
    ...
    const handleDeleteReceipt = async (receiptId: string, receiptPath: string | null) => {
        ...
        if (window.confirm('Delete submission?')) {
            // Consider using a specific loading state for delete if needed
            setLoading(true);
            try {
    ...
    if (!profile) {
        ...
        return (
            <div className="flex min-h-[calc(100vh-200px)] flex-col items-center justify-center text-yellow-400 text-center">
                ...
                <p className='text-sm text-gray-400 mt-2'>(Your profile might still be setting up. Please try refreshing.)</p>
                <button
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 text-white">
            ...
            <div>
                <AnimatePresence mode="wait">
                    {/* Pass necessary states and functions as props */}
