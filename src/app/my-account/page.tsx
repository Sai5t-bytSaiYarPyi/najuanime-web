...
// useTheme ကို import မလုပ်တော့ပါ
...
const ProfileStatsDisplay = ({ stats }: { stats: ProfileStatsData }) => {
    if (!stats) {
        // dark: prefix ဖယ်ထုတ်၊ text color ပြင်
        return <div className="bg-card p-4 rounded-lg shadow-md text-text-secondary text-sm">Loading stats...</div>;
    }
    return (
        // dark: prefix ဖယ်ထုတ်၊ text color ပြင်
        <div className="bg-card p-4 rounded-lg shadow-md">
            <h3 className="font-semibold text-text-primary mb-3 flex items-center gap-2"><BarChart2 size={16}/> Statistics</h3>
            <div className="grid grid-cols-2 gap-4 text-center">
                <div>
                    <p className="text-2xl font-bold text-accent-green">{stats.completed_count ?? 0}</p>
                    {/* dark: prefix ဖယ်ထုတ်၊ text color ပြင် */}
                    <p className="text-xs text-text-secondary uppercase flex items-center justify-center gap-1"><CheckCircle size={12}/> Anime Completed</p>
                </div>
                <div>
                    <p className="text-2xl font-bold text-yellow-400">{stats.mean_score?.toFixed(2) ?? 'N/A'}</p>
                    {/* dark: prefix ဖယ်ထုတ်၊ text color ပြင် */}
                    <p className="text-xs text-text-secondary uppercase flex items-center justify-center gap-1"><Star size={12}/> Mean Score</p>
                </div>
            </div>
        </div>
    );
...
const ProfileTabContent = ({ profile, uploadingBanner, bannerInputRef, handleImageUpload, setUploadingBanner, uploadingAvatar, avatarInputRef, setUploadingAvatar, isSubscribed, isEditingBio, setIsEditingBio, editingBioText, setEditingBioText, handleSaveBio, savingBio, isEditingUsername, setIsEditingUsername, editingUsernameText, setEditingUsernameText, handleSaveUsername, savingUsername, profileStats }: any) => {
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            {/* Banner */}
            {/* dark: prefix ဖယ်ထုတ်၊ bg/text color ပြင် */}
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 -mt-16 sm:-mt-20 px-6">
                 {/* dark: prefix ဖယ်ထုတ်၊ bg/border/text color ပြင် */}
                 <div className="relative w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-background bg-gray-600 flex items-center justify-center overflow-hidden shadow-xl shrink-0 group">
                     ...
                     {isEditingUsername ? (
                        <div className="space-y-2">
                             ...
                             <div className="relative">
                                 ...
                                 <input type="text" value={editingUsernameText} onChange={(e) => setEditingUsernameText(e.target.value)} placeholder="Enter new username" className="w-full max-w-xs p-2 pl-8 rounded bg-gray-700 border border-border text-lg font-bold focus:outline-none focus:ring-1 focus:ring-accent-green text-text-primary" maxLength={20} />
                             </div>
                             {/* dark: prefix ဖယ်ထုတ်၊ bg/text color ပြင် */}
                             <div className="flex gap-2 justify-center sm:justify-start">
                                 <button onClick={cancelEditingUsername} disabled={savingUsername} className="px-3 py-1 bg-gray-600 hover:bg-gray-500 rounded-md text-xs font-semibold disabled:opacity-50 text-text-primary"> <XCircle size={14} className="inline mr-1"/> Cancel </button>
                         ...
                         // dark: prefix ဖယ်ထုတ်၊ text color ပြင်
                        <div>
                            <h1 className="text-2xl md:text-3xl font-bold text-text-primary">{displayUsername}</h1>
                            <p className="text-text-secondary font-mono text-sm">@{profile?.naju_id || 'N/A'}</p>
                        </div>
                    )}
                </div>
            ...
            <div className="px-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                 ...
                 <div className="space-y-4">
                     ...
                     {/* dark: prefix ဖယ်ထုတ်၊ bg/text color ပြင် */}
                     <div className="bg-card p-4 rounded-lg shadow-md">
                         <h3 className="font-semibold text-text-primary mb-1">Subscription</h3>
                         {isSubscribed ? ( <div className="flex items-center gap-2"> <span className="w-3 h-3 bg-green-500 rounded-full inline-block animate-pulse"></span> <span className="text-green-400 font-medium">ACTIVE</span> <span className="text-text-secondary text-sm">(Expires: {profile?.subscription_expires_at ? new Date(profile.subscription_expires_at).toLocaleDateString() : 'N/A'})</span> </div> ) : ( <div className="flex items-center gap-2"> <span className={`w-3 h-3 ${profile?.subscription_status === 'expired' ? 'bg-red-500' : 'bg-yellow-500'} rounded-full inline-block`}></span> <span className={`${profile?.subscription_status === 'expired' ? 'text-red-400' : 'text-yellow-400'} font-medium`}> {profile?.subscription_status === 'expired' ? 'EXPIRED' : 'INACTIVE'} </span> <Link href="/subscribe" className="ml-auto text-blue-400 hover:underline text-sm font-semibold">Subscribe Now</Link> </div> )}
                    </div>
                 </div>

                 {/* Column 2: Bio */}
                  {/* dark: prefix ဖယ်ထုတ်၊ bg/text/border color ပြင် */}
                 <div className="bg-card p-4 rounded-lg shadow-md">
                    <div className="flex justify-between items-center mb-1">
                        <h3 className="font-semibold text-text-primary">About Me</h3>
                        {!isEditingBio && ( <button onClick={startEditingBio} className="text-xs text-text-secondary hover:text-white disabled:opacity-50" disabled={savingBio || savingUsername}> <Edit3 size={12} className="inline mr-1"/> Edit Bio </button> )}
                    </div>
                    {isEditingBio ? (
                        <div className="mt-2 space-y-3">
                            <textarea value={editingBioText} onChange={(e) => setEditingBioText(e.target.value)} placeholder="Tell us about yourself..." className="w-full p-2 rounded bg-gray-700 border border-border min-h-[100px] text-sm focus:outline-none focus:ring-1 focus:ring-accent-green text-text-primary" rows={4} maxLength={500} />
                            <div className="flex justify-end gap-2">
                                <button onClick={cancelEditingBio} disabled={savingBio} className="px-3 py-1 bg-gray-600 hover:bg-gray-500 rounded-md text-text-primary text-xs font-semibold disabled:opacity-50"> <XCircle size={14} className="inline mr-1"/> Cancel </button>
                    ...
                    ) : ( profile?.bio ? ( <p className="text-text-secondary text-sm whitespace-pre-wrap">{profile.bio}</p> ) : ( <p className="text-text-secondary text-sm italic">No bio added yet.</p> ) )}
                 </div>
            </div>
        </motion.div>
    );
    ...
    return (
       <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          {animeList.length > 0 ? (
              ...
              // dark: prefix ဖယ်ထုတ်၊ text/bg color တွေ ပြင်
              <div className="bg-card p-8 rounded-lg text-center shadow-md mt-6">
                  <p className="text-text-secondary">Your anime list is empty.</p>
...
// ThemeToggle component ကို ဖယ်ထုတ်၊ dark: prefix တွေ ဖယ်ထုတ်၊ text/bg/border color တွေ ပြင်
const SettingsTabContent = ({ receipts, handleDeleteReceipt }: any) => {
    // useTheme ကို မသုံးတော့ပါ
    // const { theme, setTheme, isLoading: isThemeLoading } = useTheme();

    // ThemeToggle component ကို ဖယ်ထုတ်လိုက်ပါပြီ

    return (
       <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
          <h2 className="text-2xl font-bold text-text-primary">Account Settings</h2>
           {/* Site Preferences section ကို ဖယ်ထုတ်လိုက်ပါပြီ */}
           {/* <div className="bg-card p-6 rounded-lg shadow-md"> ... </div> */}

           {/* dark: prefix ဖယ်ထုတ်၊ text/bg color တွေ ပြင် */}
           <div className="bg-card p-6 rounded-lg shadow-md"><h3 className="text-xl font-semibold mb-3 text-text-primary">Edit Profile</h3><p className="text-text-secondary text-sm">(Coming Soon)</p></div>
           <div className="bg-card p-6 rounded-lg shadow-md"><h3 className="text-xl font-semibold mb-3 text-text-primary">Change Password</h3><p className="text-text-secondary text-sm">(Coming Soon)</p></div>
           <div className="bg-card p-6 rounded-lg shadow-md">
               <h3 className="text-xl font-semibold mb-4 text-text-primary">My Receipt Submissions</h3>
               <div className="space-y-3 max-h-60 overflow-y-auto pr-2">
                   {receipts.length > 0 ? receipts.map((r: Receipt) => (
                      ...
                      <div key={r.id} className="p-3 bg-gray-700 rounded-md flex justify-between items-center gap-4">
                          <div>
                              <p className="text-sm text-text-secondary">Submitted: {new Date(r.created_at).toLocaleString()}</p>
                          ...
                          {/* isThemeLoading ကို စစ်ဆေးစရာမလိုတော့ပါ */}
                          {(r.status === 'pending' || r.status === 'rejected') && ( <button onClick={() => handleDeleteReceipt(r.id, r.receipt_url)} className="text-sm bg-red-600 hover:bg-red-700 px-3 py-1 rounded text-white font-medium transition-colors disabled:bg-gray-500 disabled:cursor-not-allowed"> Delete </button> )}
                      </div>
                  )) : <p className="text-text-secondary">No submission history.</p>}
               </div>
            ...
            {/* dark: prefix ဖယ်ထုတ်၊ text/bg/border color တွေ ပြင် */}
...
export default function MyAccountPage() {
    ...
    // Theme context ကို မသုံးတော့ပါ
    // const { theme, isLoading: isThemeLoading } = useTheme();

    // --- setupUser, checkSessionAndSetup, Auth Listener, Other Handlers (ယခင်အတိုင်း၊ console.log တွေ ဖယ်ထုတ်နိုင်သည်) ---
    ...
    // handleDeleteReceipt, handleImageUpload, handleSaveBio, handleSaveUsername functions remain the same
    const handleDeleteReceipt = async (receiptId: string, receiptPath: string | null) => {
        if (!receiptId) return;
        if (window.confirm('Are you sure you want to delete this receipt submission? This cannot be undone.')) {
            ...
            try {
                ...
                // 2. If a path exists, delete the file from storage
                if (receiptPath) {
                    const { error: storageError } = await supabase.storage
                        .from('receipts')
                        .remove([receiptPath]);
                    // Log storage error but don't necessarily throw, maybe the file was already gone
                    if (storageError) console.warn(`Could not delete receipt file from storage (${receiptPath}): ${storageError.message}`);
                }
                ...
                setError(`Failed to delete receipt: ${err.message}. Please try again.`);
    ...
    const handleImageUpload = async ( event: React.ChangeEvent<HTMLInputElement>, bucket: 'avatars' | 'banners', setLoadingState: (loading: boolean) => void ) => {
        ...
        try {
            // Check if user already has an image in this bucket (optional cleanup)
            const { data: listData, error: listError } = await supabase.storage .from(bucket) .list(session.user.id, { limit: 10 });
             if (listError) console.warn(`Could not list old ${bucket}:`, listError.message);
             if (listData && listData.length > 0) { const oldFiles = listData.map(f => `${session.user.id}/${f.name}`); console.log(`Removing old ${bucket}:`, oldFiles); await supabase.storage.from(bucket).remove(oldFiles); }

            // Upload new image
            const { error: uploadError } = await supabase.storage .from(bucket) .upload(filePath, file, { upsert: false }); // Use upsert: false to avoid overwriting unrelated files if needed
            if (uploadError) throw uploadError;

            // Get public URL (adjust based on your bucket's public/private status)
            const { data: urlData } = supabase.storage.from(bucket).getPublicUrl(filePath);
            if (!urlData.publicUrl) throw new Error("Could not get public URL for uploaded image.");
            const imageUrl = urlData.publicUrl;

            // Update profiles table
            const updateData = bucket === 'avatars' ? { avatar_url: imageUrl } : { banner_url: imageUrl };
            const { error: updateError } = await supabase .from('profiles') .update(updateData) .eq('id', session.user.id);
            if (updateError) throw updateError;

            // Update local profile state
            setProfile(prev => prev ? { ...prev, ...(bucket === 'avatars' ? { avatar_url: imageUrl } : { banner_url: imageUrl }) } : null);
            alert(`${bucket === 'avatars' ? 'Avatar' : 'Banner'} updated successfully!`);

        } catch (err: any) { console.error(`Error uploading ${bucket}:`, err); setError(`Failed to upload ${bucket}: ${err.message}`); alert(`Error: ${err.message}`);
    ...
    const handleSaveBio = async (newBio: string) => {
        ...
        try {
            const { error } = await supabase .from('profiles') .update({ bio: trimmedBio || null }) // Send null if empty
            .eq('id', session.user.id);
            if (error) throw error;
            setProfile(prev => prev ? { ...prev, bio: trimmedBio || null } : null);
            setIsEditingBio(false);
        } catch (err: any) { console.error('Error saving bio:', err); setError(`Failed to save bio: ${err.message}`); alert(`Error: ${err.message}`);
        } finally { setSavingBio(false); }
    };
     const handleSaveUsername = async (newUsername: string) => {
        if (!session?.user) return;
        const trimmedUsername = newUsername.trim();
        // Basic client-side validation (can add more checks)
        if (trimmedUsername.length < 3 || trimmedUsername.length > 20) { setError("Username must be between 3 and 20 characters."); return; }
        const usernameRegex = /^[a-zA-Z0-9_-]+$/;
        if (!usernameRegex.test(trimmedUsername)) { setError("Username can only contain letters, numbers, underscores (_), and hyphens (-)."); return; }

        setSavingUsername(true); setError(null);
        try {
            // Check if username already exists (using RPC for case-insensitive check is better but requires setup)
            // Simple check here:
             const { data: existingUser, error: checkError } = await supabase .from('profiles') .select('id') .ilike('naju_id', trimmedUsername) // Case-insensitive check
             .neq('id', session.user.id) // Exclude self
             .limit(1);
             if (checkError) throw new Error(`Could not verify username uniqueness: ${checkError.message}`);
             if (existingUser && existingUser.length > 0) { throw new Error(`Username "${trimmedUsername}" is already taken.`); }

            // Update the username
            const { error: updateError } = await supabase .from('profiles') .update({ naju_id: trimmedUsername }) .eq('id', session.user.id);
            if (updateError) {
                 // Handle potential race condition or DB constraint error
                 if (updateError.message.includes('duplicate key value violates unique constraint')) { throw new Error(`Username "${trimmedUsername}" is already taken.`); }
                 throw updateError;
             }
            setProfile(prev => prev ? { ...prev, naju_id: trimmedUsername } : null);
            setIsEditingUsername(false);
            alert('Username updated successfully!');
        } catch (err: any) { console.error('Error saving username:', err); setError(`Failed to save username: ${err.message}`); alert(`Error: ${err.message}`);
        } finally { setSavingUsername(false); }
    };

    // --- RENDER LOGIC (dark: prefix တွေ ဖယ်ထုတ်၊ text/bg color တွေ ပြင်) ---
    if (loading) { return (<div className="flex min-h-[calc(100vh-200px)] items-center justify-center text-white"><Loader className="animate-spin mr-2" size={24} /> Loading Account Data...</div>); }
    ...
    // console.log("Rendering main account content without theme context");
    ...
    return (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 text-text-primary">
            {/* Tab Navigation */}
             {/* dark: prefix ဖယ်ထုတ်၊ border/text color ပြင် */}
