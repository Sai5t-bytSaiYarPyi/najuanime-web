...
import { Loader, AlertTriangle, User as UserIcon, ListVideo, Settings, Edit3, UploadCloud, Save, XCircle, AtSign, BarChart2, CheckCircle, Star } from 'lucide-react'; // <-- Icons အသစ်တွေ ထည့်ပါ
...
// --- START: Stats Data Type ---
type ProfileStatsData = {
    completed_count: number;
    mean_score: number;
} | null; // Initial state အတွက် null ပါ ထည့်ပါ
// --- END: Stats Data Type ---

// --- START: ProfileStatsDisplay Component ---
const ProfileStatsDisplay = ({ stats }: { stats: ProfileStatsData }) => {
    if (!stats) {
        // Handle loading or error state for stats if needed separately
...
// --- END: ProfileStatsDisplay Component ---


// --- ProfileTabContent ကို Stats Component ထည့်ရန် ပြင်ဆင် ---
const ProfileTabContent = ({
    ...
    // Stats prop
    profileStats // <-- Stats data ကို လက်ခံရန်
}: any) => {
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 -mt-16 sm:-mt-20 px-6">
                ...
                <div className="text-center sm:text-left pb-2 flex-grow">
                     {isEditingUsername ? ( /* ... username edit form ... */
            ...
            <div className="px-6 grid grid-cols-1 md:grid-cols-2 gap-4"> {/* Grid Layout ပြောင်း */}
                 ...
                 <div className="space-y-4">
                     ...
                     <div className="bg-card-dark p-4 rounded-lg shadow-md">
                         ...
                         {isSubscribed ? ( /* ... active ... */ <div className="flex items-center gap-2"> <span className="w-3 h-3 bg-green-500 rounded-full inline-block animate-pulse"></span> <span className="text-green-400 font-medium">ACTIVE</span> <span className="text-gray-400 text-sm">(Expires: {profile?.subscription_expires_at ? new Date(profile.subscription_expires_at).toLocaleDateString() : 'N/A'})</span> </div> ) : ( /* ... inactive ... */ <div className="flex items-center gap-2"> <span className={`w-3 h-3 ${profile?.subscription_status === 'expired' ? 'bg-red-500' : 'bg-yellow-500'} rounded-full inline-block`}></span> <span className={`${profile?.subscription_status === 'expired' ? 'text-red-400' : 'text-yellow-400'} font-medium`}> {profile?.subscription_status === 'expired' ? 'EXPIRED' : 'INACTIVE'} </span> <Link href="/subscribe" className="ml-auto text-blue-400 hover:underline text-sm font-semibold">Subscribe Now</Link> </div> )}
                    </div>
                 </div>
    ...
    const setupUser = useCallback(async (user: User) => {
        ...
        try {
            ...
            const [profileResponse, receiptsResponse, animeListResponse, statsResponse] = await Promise.all([
                ...
                supabase.rpc('get_user_profile_stats', { p_user_id: user.id }) // <-- RPC Call
            ]);

            console.log("Profile response:", profileResponse); console.log("Receipts response:", receiptsResponse); console.log("Anime list response:", animeListResponse); console.log("Stats response:", statsResponse); // <-- Log stats
            ...
            if (statsResponse.error) throw new Error(`Stats fetch failed: ${statsResponse.error.message}`); // <-- Stats error check

            // State Updates (unchanged for profile, receipts, animeList)
            ...
            // Set Profile Stats state
            setProfileStats(statsResponse.data as ProfileStatsData); // <-- Update stats state
            ...
            setProfile(null); setReceipts([]); setAnimeList([]); setProfileStats(null); // <-- Reset stats on error
            return false;
        }
    }, []);
    // --- END: setupUser တွင် Stats RPC ကို ခေါ်ရန် ပြင်ဆင် ---
                ...
                <AnimatePresence mode="wait">
                    ...
                    {activeTab === 'profile' &&
                        <ProfileTabContent
                            ...
                            // Stats prop
                            profileStats={profileStats} // <-- Pass stats data
                        />
                    }
                    {/* --- END: Pass profileStats --- */}
